...
console.log(`
𝄢 · • —– ٠ ✤ ٠ —– • · · • —– ٠ ✤ ٠ —– • ·✧༄
❚ ▸ ${chalk.cyan('QUEEN-SAKURA_MD ❱❱')} ${chalk.bgMagenta(chalk.white.italic('%s'))}
❚ ▸ ${chalk.cyan('SCHEDULE ❱❱')} ${chalk.black.bgWhite('%s')}
...
𝄢 · • —– ٠ ✤ ٠ —– • · · • —– ٠ ✤ ٠ —– • ·✧༄`.trim(),          
me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' ║ *QUEEN-SAKURA_MD* ║'}`,
...
case 26:
return 'Who can send messages in the group has been configured'
case 27:
return `${name_user} has joined the group`
case 28:
return `${name_user} has been removed from the group`
case 29:
return `${name_user} is new admin. of the group
case 30:
return `${name_user} is no longer admin. of the group
case 31:
return `${name_user} has been invited to the group`
case 32:
return `${name_user} has left the group`
case 145:
return `"approve new members" was configured on the group`
case 171:
return `"add other members" was set to the group`
default:
return messageStubType //'None'
}}
